---
paths:
  - "internal/outage/subscription/**/*.go"
---

# Subscription Rules

- `internal/outage/subscription` owns the subscription conversation workflow and application-level subscription operations.
...
- Return structured `Response` values (including all Ukrainian text) for adapters to render; keep Telegram bot API calls, reply-keyboard construction, and transport details in `internal/outage/telegram`.
- Use `internal/outage/users` entities, validation, repositories, and matching behavior instead of duplicating user/address/street rules here.
