...
func TestFileUserRepository_AtomicWrite(t *testing.T) {
	...
	tmpPath := filepath.Join(repo.store.Dir, "12345.toml.tmp")
	_, err = os.Stat(tmpPath)
	assert.True(t, os.IsNotExist(err))

	// Verify the actual file exists
	actualPath := filepath.Join(repo.store.Dir, "12345.toml")
...
func TestFileUserRepository_FindAllSkipsMalformedFiles(t *testing.T) {
	...
	malformedPath := filepath.Join(dir, "222.toml")
	require.NoError(t, os.WriteFile(malformedPath, []byte("not valid toml [[["), 0o644))
...
func TestFileUserRepository_LoadFromFile_InvalidTOML(t *testing.T) {
	dir := t.TempDir()
	repo, err := NewFileUserRepository(dir)
	require.NoError(t, err)

	badPath := filepath.Join(dir, "999.toml")
...
func TestFileUserRepository_LoadFromFile_InvalidAddress(t *testing.T) {
	...
	badTOML := "street_id = 0\nstreet_name = \"Test\"\nbuilding = \"10\"\n"
	badPath := filepath.Join(dir, "999.toml")
	require.NoError(t, os.WriteFile(badPath, []byte(badTOML), 0o644))
