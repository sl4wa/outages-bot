...
func (m *mockSender) Send(userID int64, content Content) error {
...
type mockProvider struct {
	outages []outage.RawOutage
	err     error
}

func (m *mockProvider) FetchOutages(_ context.Context) ([]outage.RawOutage, error) {
	return m.outages, m.err
}

func makeTestOutage(streetID int, buildings []string) outage.RawOutage {
	return outage.RawOutage{
		ID:         1,
...
func newNotifyUsers(provider *mockProvider, sender *mockSender, repo *mockUserRepo, logger *log.Logger) *NotifyUsers {
	fetchService := outage.NewFetchOutages(provider)
...
func newNotifyUsersWithSnapshot(provider *mockProvider, sender *mockSender, repo *mockUserRepo, snap outage.SnapshotStore, logger *log.Logger) *NotifyUsers {
	fetchService := outage.NewFetchOutages(provider)
...
func TestNotifyUsers_Snapshot_ChangedSnapshot_SavesAndNotifies(t *testing.T) {
	...
	secondOutages := []outage.RawOutage{makeTestOutage(1, []string{"10", "12"})}
...
func TestNotifyUsers_Snapshot_Miss_LogsChanged(t *testing.T) {
	...
	provider.outages = []outage.RawOutage{makeTestOutage(1, []string{"10", "12"})}
	buf.Reset()
