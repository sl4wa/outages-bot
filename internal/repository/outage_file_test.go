...
func makeOutage(streetID int, streetName string, buildings []string, start, end time.Time, comment string) *domain.Outage {
	period, _ := domain.NewOutagePeriod(start, end)
	addr, _ := domain.NewOutageAddress(streetID, streetName, buildings, "Львів")
...
func TestFileOutageRepository_SaveAndLoad_RoundTrip(t *testing.T) {
	...
	want := []*domain.Outage{makeOutage(1, "Стрийська", []string{"10", "12"}, t0, t1, "test")}
...
func TestFileOutageRepository_Save_IsAtomic(t *testing.T) {
	...
	outages := []*domain.Outage{makeOutage(1, "Стрийська", []string{"1"}, t0, t1, "x")}
...
func TestFileOutageRepository_UTCNormalization(t *testing.T) {
	...
	outages := []*domain.Outage{makeOutage(1, "Стрийська", []string{"10"}, t0.In(kyiv), t1.In(kyiv), "c")}
	require.NoError(t, repo.Save(outages))
